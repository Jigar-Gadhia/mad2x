...
const Profile_Edit = navigation => {
  ...
  const ApiPost = async () => {
    ...
    formdata.append('mobile', array[2].value || null);
    formdata.append('age', array[3].value.toString() || null);
    formdata.append('address', array[4].value.toString() || null);
    formdata.append('profilePic', {
      uri: url, // this should be a file URI (e.g., from image picker)
      ...
      <View style={{height: hp('100%')}}>
        ...
        <View style={Styles.bg}>
          <View style={Styles.back_view}>
            {edit == false ? (
              <TouchableOpacity
                onPress={() =>
                  navigation.navigation.dispatch(
                    CommonActions.reset({routes: [{name: 'Profile'}]}),
                  )
                }>
                <Icon
            ...
            <TouchableOpacity
              onPress={toggleEdit}
              style={{marginLeft: wp('58%')}}>
              {edit == false ? (
                <Icon name="create-outline" size={32} color={Colors.white} />
              ) : (
                <Icon
          ...
          <View style={Styles.profile_bg}>
            <View
              style={[Styles.bg_image, edit == true ? {elevation: 10} : null]}>
              <ImageBackground
                source={{uri: url}}
                resizeMode="center"
                style={{height: hp('15%')}}
