          ...
          return (
            <View style={{flex: 1, flexDirection: 'row'}}>
              <TouchableOpacity
                style={
                  active == index
                    ? Styles.active_slot_button
                    : Styles.slot_button
                }
                onPress={setActive(index)}>
                <Text
                  style={
                    active == index ? Styles.active_slot_text : Styles.slot_text
                  }>
                  {item.time}
                </Text>
              </TouchableOpacity>
    ...
    return (
      <TouchableOpacity
        ...
        style={{
          ...
          height: hp('3%'),
          backgroundColor: select == index ? scheme.blue : null,
        }}
        ...
        <Text
          style={{
            ...
            color: select == index ? scheme.white : scheme.dark_grey,
          }}
          key={item.name}>
          {item.abbr}
        </Text>
  ...
  return (
    <ScrollView style={Styles.bg}>
      ...
      <View style={{flexDirection: 'row'}}>
        <TouchableOpacity
          style={Styles.years_btn}
          onPress={() => setValue(!value)}>
          <Text style={{color: scheme.black}}>
            {ind == '' ? 'Month' : month[ind]}
          </Text>
          <Icon
            name={value == true ? 'chevron-up-outline' : 'chevron-down-outline'}
            size={20}
            color={scheme.black}
          />
        </TouchableOpacity>
        <TouchableOpacity
          ...
          onPress={() => setY(!yvalue)}>
          <Text style={{color: scheme.black}}>
            {yind == '' ? 'Year' : year_name[yind].yname}
          </Text>
          <Icon
            name={
              yvalue == true ? 'chevron-up-outline' : 'chevron-down-outline'
            }
            size={20}
            color={scheme.black}
          />
      ...
      <View style={Styles.time_view}>
        <View style={Styles.time_header}>
          <FlatList
            ...
            renderItem={(item, index) => {
              return (
                <View style={{flex: 1, marginBottom: hp('2%')}}>
                  <View style={Styles.time_header}>
                    <Text style={Styles.time_header_text} key={index}>
                      {item.item.slot}
                    </Text>
                  </View>
                      ...
                      return (
                        <View>
                          <TouchableOpacity
                            ...
                            onPress={() => setActive(item.select)}>
                            <Text
                              style={
                                active == item.select
                                  ? Styles.active_slot_text
                                  : Styles.slot_text
                              }
                              key={index}>
                              {item.time}
          ...
          />
          <TouchableOpacity
            style={Styles.appoint_btn}
            onPress={() =>
              Alert.alert(
                'Desclaimer',
                'Appointment feature is under development !',
              )
            }>
